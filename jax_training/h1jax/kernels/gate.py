"""h1jax TPU profile gate: measure real Falcon-H1 training throughput on v5e-8, driven by a run spec.

This is a measurement kernel, not a training run. For each requested shape it compiles the full
FP32-master / BF16-compute AdamW step under explicit SPMD sharding (parameters and optimizer state
replicated or FSDP-sharded, batch sharded across the eight chips), records XLA's memory and cost
analysis BEFORE executing, then times warm steps with asynchronous dispatch. It also micro-benchmarks
the step's components (attention, Mamba mixer, SSD scan, MLP, embedding/head/loss, and a plain matmul
roofline) so the time budget is decomposed without needing xprof, writes a short profiler trace for
offline analysis, checks the base checkpoint's validation loss (against an hbox Transformers
measurement when the spec carries one), and finishes with a few real-learning-rate steps to confirm
the loss moves.

This is the body of the former `kaggle/tpu_h1jax_profile_gate_15b_deep/run.py` as `main(spec)`: every
`HGHOST_GATE_*` / `HGHOST_*` setting is `spec[<name lower-cased>]` with the same default
(`common.GATE_SETTINGS`); `HGHOST_LOCAL` rehearsal mode is `spec["local"]` or the environment variable.
"""
...
def main(spec: dict) -> None:
    ...
    SHAPES = S["shapes"]
    WARMUP_STEPS = S["warmup"]
    TIMED_STEPS = S["steps"]
    SYNC_STEPS = S["sync_steps"]
    PROFILE_STEPS = S["profile_steps"]
    BENCH_ITERS = S["bench_iters"]
    SANITY_STEPS = S["sanity_steps"]
    WATCHDOG_MINUTES = S["watchdog_minutes"]
    HARD_MAX_MINUTES = S["max_minutes"]

    _WATCHDOG = start_watchdog(WATCHDOG_MINUTES, HARD_MAX_MINUTES)  # noqa: F841 (lives as long as this process)
    SANITY_LR = S["sanity_lr"]
    ...
    PEAK_FLOPS_PER_CHIP = S["peak_flops_per_chip"]
    HBM_BYTES_PER_CHIP = S["hbm_bytes_per_chip"]
    # The hbox Transformers number for the base checkpoint on the fixed 32-sequence slice, when one exists for
    # this model and tokenization (research/results/hbox-cpt-10m.md has the 91M one); None = no comparison.
    HBOX_BASE_EVAL: dict[str, float] | None = (
        {"loss": S["hbox_base_loss"], "accuracy": S["hbox_base_accuracy"]}
        if S["hbox_base_loss"] is not None
        else None
    )
    ...
    for shape in shape_specs:
        per_chip, seq_len, remat = shape["per_chip"], shape["seq_len"], shape["remat"]
        ...
        try:
            ...
            memory_probe(f"before_compile_{shape['name']}")
            compile_started = time.perf_counter()
            lowered = train_step.lower(params, opt_state, first)
            compiled = lowered.compile()
            memory_probe(f"after_compile_{shape['name']}")
            ...
            if PROFILE_STEPS > 0:
                trace_dir = OUTPUT / "profile" / shape["name"]
                try:
