...
J-lens use cases"), for the h1jax Falcon-H1 checkpoints (JAX on the Mac CPU, plus a parity-checked PyTorch
port for hbox's GPU), with the first experiments on the 90M base / 91M library-leaf pair.
**The 0.5B pair was not run**: the 0.5B lens computation was stopped on the evening of
2026-09-03 by decision (electricity; the 91M result answers the question) with the base
at context 51/84 and e2-v4 not started; only its sanity check and the JAX/PyTorch parity
context exist. Nothing here is committed; all outputs live under
`research/jlens/out/<model>/`.
...
- **hbox, PyTorch 2.9.1+rocm6.3 on the Radeon (12.9 GB VRAM)**, `jlens_torch.py`:
  ...
  **0.5B 60 s at T = 64, 83-87 s at T = 80-90, 110-120 s at T = 97-128** (51 Jacobians in
87 min before the run was stopped; the room prompts at T ~ 190 were never reached). The Triton SSD
...
layer, seed 0) for both 91M models at l = 8, 12, 16. Five of the six 91M lenses were
computed on the Mac (JAX); the 91m-leaf l = 16 lens on hbox (PyTorch, same contexts) after
the Mac run of that layer was killed at 21/84 - the two implementations agree to 2e-6 on a
full Jacobian, so the six are treated as one set. With
...
## Results (completed measurements only)

Models: `90m-base` = Falcon-H1-Tiny-90M-Base (`kaggle/base_model_dataset_public`),
`91m-leaf` = library leaf-s1-e4 (`artifacts/checkpoints/tpu/h-ghost-h1jax-leaf-s1-e4/leaf-s1-e4-decay10/tokens-001535061369`).
The 0.5B rows: **not run (stopped 2026-09-03 evening by decision; 91M answered the
question)** - only the `check` row (sanity) and the parity context exist for `05b-base`;
nothing for `05b-e2v4`.

Tables are generated by `report.py` from `out/<model>/*.json` (`layer` is the residual
stream entering that decoder layer; L = 24 for both models).
...
| model | layer | lens | readout | sparse |
|---|---|---|---|---|
| 05b-base | 18 | not run (stopped 2026-09-03 evening by decision; 91M answered the question) - hbox run reached 51/84 contexts, no lens written | not run | not run |
| 05b-e2v4 | 18 | not run (stopped 2026-09-03 evening by decision; 91M answered the question) | not run | not run |

What does exist for the 0.5B: the sanity row above (`check`, JAX), and the JAX/PyTorch
parity files `out/05b-base/parity-L18*.{json,npy}` and `parity-torch-L18-*.json`
(rel. Frobenius error 2.3e-6 reference kernel, 2.6e-6 Triton).
...
nothing in the two 91M models behaves like a reportable, modulable workspace, and the
library-continued-pretrained leaf does not differ from its base in that respect. (The
0.5B pair was not measured, see above.)
...
was in scope, and the 0.5B pair (base vs the library-CPT e2-v4) was stopped before any
lens completed.
...
generalization* is what `inject` tests and it fails in the specific sense above: at l = 8
and 12 the lens direction is no better an argument to the "capital of" function than the
raw unembedding direction, at the leaf's l = 16 it is somewhat better, and in no case does
the function produce the right value (top-1 among capitals at most 0.18 vs the 0.08
baseline).
